...
# Запускаем веб-оболочку для удобной работы через браузер
...
| **web-shell**          | Запускает веб-оболочку для работы с HH.RU API через браузер. Удобный веб-интерфейс для выполнения всех операций. См. [WEB_SHELL.md](WEB_SHELL.md) |
